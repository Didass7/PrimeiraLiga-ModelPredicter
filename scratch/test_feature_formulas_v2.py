...
for season in df['Epoca'].unique():
    ...
    for idx, row in df_s.iterrows():
        ...
        # Calculate priors
        if season == '2010-2011' and (home not in team_goals_scored or away not in team_goals_scored):
            # First season fallbacks
            calc_casa_xg = 1.3
            calc_visi_xg = 1.1
        elif home not in team_goals_scored or away not in team_goals_scored:
            # Other seasons: if it is Jornada 1 or either team has no games yet in this season
            # We check if they have historical att/def
...
print("\nExpected Goals Verification (with 2010-2011 fallback):")
...
if (err_casa_xg > 1e-5).sum() > 0:
    ...
    print(mismatch_df[['Epoca', 'Jornada', 'Equipa_Casa', 'Equipa_Visitante', 'Casa_ExpectedGolos', 'Calc_Casa_ExpectedGolos']].head(10))
